...
cpSync(resolve(root, 'server/onamae/api/contact.php'), resolve(out, 'api/contact.php'))
cpSync(resolve(root, 'server/onamae/api/contact-config.example.php'), resolve(out, 'api/contact-config.example.php'))

console.log(`package-onamae: ${out} に公開ファイル一式を出力しました`)
console.log('  サーバー上で api/contact-config.example.php を api/contact-config.php にコピーし、送信先を記入してください')
